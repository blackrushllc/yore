...
For local development, scripts are provided in the `localdev/` directory:

- `localdev/serve.php`: Launches a PHP built-in server that mimics production routing and tenancy. Usage:
  - `composer serve <domain>` (or run directly with `php localdev/serve.php <domain>`)
  - Example: `composer serve app.firsthealthhc.com`
  - If no domain is provided, you will be prompted for one.
- `localdev/router.php`: Used by the local server to simulate Apache rewrite rules and domain-based tenancy. This is **not used in production**.

These scripts are for local development only and should not be used in production. The local server will behave like your production environment, including domain-based routing and tenancy logic.

Make sure your Composer scripts reference `localdev/serve.php` if you use `composer serve`.
