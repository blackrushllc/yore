...
class Module extends Modules {
    ...
    function init() {
        ...
        // Check if unix_socket is specified in settings (if set, it will be added to DSN)
        if (isset($this->controller->settings['database_module_unix_socket'])) {
            $this->unix_socket = $this->controller->settings['database_module_unix_socket'];
        }

        // Build DSN
        // If unix_socket is set to empty string, force TCP/IP by using 127.0.0.1 instead of the host value
        if ($this->unix_socket === '') {
            // Force TCP/IP connection by using 127.0.0.1 instead of localhost
            $dsn = "mysql:host=127.0.0.1;dbname=$this->database;charset=$this->charset;port=$this->port";
        } else if ($this->unix_socket !== null && $this->unix_socket !== '') {
            // Use explicit unix socket path
            $dsn = "mysql:unix_socket=$this->unix_socket;dbname=$this->database;charset=$this->charset";
        } else {
            // Default behavior - use host and port
    ...
    function uploadFileToDatabase($file, $code, $hospital_name) {
        ...
        // Strip the BOM from fileData
        $fileData = ltrim($fileData, "\xEF\xBB\xBF");
